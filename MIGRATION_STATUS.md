...
| New admin panel (`admin/`) | **Supabase** | Live: staff login/RBAC gate, Dashboard (real KPIs), Bookings (list/search/filter/detail/stage-update), Booking Tracker (Today/Upcoming/Pipeline/All). |
| New admin panel — everything else | **Not built yet** | Tests/Packages/Coupons management, User/Staff management, Reports, Cash Flow, Activity Log, Settings. See [ADMIN_MIGRATION_MAP.md](ADMIN_MIGRATION_MAP.md) for the full breakdown. |
